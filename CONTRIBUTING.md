...
- **`git2`** is a dependency with `default-features = false` (see `ccm-index/Cargo.toml`) — it skips the `ssh`/`https` transports and their system OpenSSL/libssh2 requirement, since this project only reads local repo state (blob hashing), never clones/fetches/pushes. No OpenSSL install needed on any OS for this reason.
