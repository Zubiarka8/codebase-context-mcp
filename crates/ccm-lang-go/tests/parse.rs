...
fn parse(src: &str) -> ccm_core::ParsedFile {
    GoParser
        .parse(&SourceFile { relative_path: "calc.go".to_string(), contents: src.to_string() })
...
fn extracts_function_and_call() {
    ...
    assert_eq!(add.parent.as_deref(), Some("calc"), "top-level function's parent is its package");
...
fn method_with_pointer_receiver_attaches_to_its_type() {
    ...
    let invoice = parsed.symbols.iter().find(|s| s.name == "Invoice" && s.kind == SymbolKind::Struct).unwrap();
    ...
    assert_eq!(add_item.parent.as_deref(), Some("Invoice"), "pointer receiver `*Invoice` must unwrap to the plain type name");
}
...
fn interface_and_its_declared_methods_are_indexed_without_resolving_implementers() {
    ...
    let perimeter = parsed.symbols.iter().find(|s| s.name == "Perimeter").unwrap();
    ...
    assert!(parsed.relations.iter().all(|r| r.kind != RelationKind::Implements));
}

#[test]
fn extracts_package_and_import() {
    let parsed = parse("package billing\n\nimport \"fmt\"\n\nfunc noop() {\n\tfmt.Println(\"hi\")\n}\n");
    assert!(parsed.symbols.iter().any(|s| s.name == "billing" && s.kind == SymbolKind::Module));
