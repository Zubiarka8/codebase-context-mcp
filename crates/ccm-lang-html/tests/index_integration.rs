...
//! relations actually resolve against `Rule` symbols that `ccm-lang-css`
//! indexed from a *different file* — not just that both parsers run without
//! error side by side (that's what `ccm-mcp-server`'s `polyglot_fixture.rs`
