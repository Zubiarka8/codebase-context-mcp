//! Standalone Lua token-benchmark, mirroring `ccm-cli`'s
...
//! kept local to this crate: `ccm-lang-lua` is deliberately not wired into
//! `ccm-cli`'s or `ccm-mcp-server`'s production `LanguageRegistry` (see this
...
//! benchmark does not add it as a `ccm-cli` dependency either. Only the
//! measurement method is duplicated; the fixture and result are real.
//!
//! Usage: cargo run -p ccm-lang-lua --example token_benchmark_lua
//!
//! Named `token_benchmark_lua` (not `token_benchmark`) to avoid an
//! example-binary filename collision with `ccm-cli`'s own
