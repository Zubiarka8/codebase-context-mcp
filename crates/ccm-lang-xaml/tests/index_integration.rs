//! Runs the real `ccm-index` pipeline against a small 2-file, 2-language
...
//! that `ccm-lang-csharp` indexed from a *different file* — not just that
//! both parsers run without error side by side.

// Test code: an unwrap()/expect() here means a broken test precondition, and
// panicking is the correct behavior — this is not production code parsing
// untrusted repo content (see crates/ccm-lang-xaml/src/ for that policy).
...
use ccm_lang_csharp::CSharpParser;
use ccm_lang_xaml::XamlParser;
